...
export function getPaymentCredentials() {
  return {
    publicKey: env('PAYMENT_PUBLIC_KEY') || env('FREEPAY_PUBLIC_KEY'),
    secretKey: env('PAYMENT_SECRET_KEY') || env('FREEPAY_SECRET_KEY')
  };
}
...
function getGatewayApi() {
  const url = env('PAYMENT_API_URL') || env('FREEPAY_API_URL') || 'https://api.freepaybrasil.com';
  if (!url) {
...
export function mapPaymentStatus(status) {
  ...
  if ([
    'paid', 'approved', 'approved_payment', 'completed', 'complete',
    'succeeded', 'success', 'confirmed', 'pago', 'aprovado'
  ].includes(normalized)) return 'paid';
  if (['refused', 'rejected', 'cancelled', 'canceled', 'failed', 'expired'].includes(normalized)) return 'failed';
...
export function getWebhookUrl() {
  const explicit = env('PAYMENT_WEBHOOK_URL') || env('FREEPAY_POSTBACK_URL');
